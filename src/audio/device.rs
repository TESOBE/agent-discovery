...
/// Play a descending two-tone alert to signal OBP connectivity loss.
/// Uses a descending minor interval (E4 → C4) — opposite of the rising handshake chime.
pub fn play_no_obp_tone() -> Result<()> {
    let sample_rate = modulator::SAMPLE_RATE;
    let host = cpal::default_host();
    let device = host
        .default_output_device()
        .context("No output audio device for alert tone")?;
    ...
    // Two notes: E4 (329.63Hz) for 150ms, then C4 (261.63Hz) for 200ms
    ...
    let gap_samples = (sample_rate as f32 * 0.04) as usize; // 40ms gap
    let total = note1_samples + gap_samples + note2_samples;

    let tau = std::f32::consts::TAU;
    let mut samples = Vec::with_capacity(total);

    // Note 1 (E4) with envelope
    for i in 0..note1_samples {
        let t = i as f32 / sample_rate as f32;
        let env = smooth_envelope(i, note1_samples);
        samples.push((tau * note1_freq * t).sin() * 0.35 * env);
    ...
    let stream = device.build_output_stream(
        ...
        |err| tracing::error!("Alert tone output error: {}", err),
        None,
    )?;
